...
void receive_messages() {
  ...
  do {
	...
	cerr << "Resetted cerr!\n";
      ...
      // handle problem with empty lines readen from file
	...
	cerr << "Read command from file " << file_loader.getfilename()
      ...
      if (*message == '#') continue;// Message is comment
	...
	if (p.size() < 2) {
      ...
      if (clr) {
	// First argument current not used
	if ((*clr)(0, cpu, cerr, p)) continue;
	...
	if (!(comm->cpu_is_thinking)) {
	  ...
	  cerr << " - delaying responce until cpu has moved...\n";
	}
	...
	//} else if (strcmp(prefix, "dir") == 0) {
	//cpu->print_board(cerr, 2);
	//debug_message("print board");
...
bool debug_message(string message) {
  //cerr << "#("; for (unsigned int i=0; i<p.size(); i++) cerr << p[i] << " "; cerr << ")#\n";
