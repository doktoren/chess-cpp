// Enforce correct indentation in emacs:
#define aalskhlf
aalskhlf{
  Board2 board;
  ...
  for (int player=0; player<(symmetric_endgame ? 1 : 2); player++) {
    ...
    map<int, EFState> old_states = load_dependency(player);
	...
	case GAME_OPEN:
	  {
	    int best_value = 0;// remember 0 represents a lost positions, worst possible
  ...
  for (int player=0; player<(symmetric_endgame ? 1 : 2); player++) {
    ...
    move_count[player] = new uchar[table_size];
	...
	bool reduced_symmetry = !pawnless_endgame ? false :
	...
	if (board.calc_game_status() == GAME_OPEN) {
	  ...
	  while (board.next_move(move)) {
	    if (!(move.is_en_passant()  ||  move.is_pawn_promotion()  ||  board[move.to])) {
	      ...
	      bool reduced_symmetry_dest = !pawnless_endgame ? false :
		(SAME_DIAGONAL[board.get_king_position(0)] ==
		 SAME_DIAGONAL[board.get_king_position(1)]);
  ...
  for (int n=0; n<=max_mate_depth; n++) {
    ...
    for (int _player=0; _player<2; _player++) {
      int n_win = symmetric_endgame ? (_player ? n+1 : 9999) : (n+_player);
      if (n_win==0) n_win = 9999;
      int n_loss = (symmetric_endgame && _player) ? 9999 : -n;
	...
	if (table[player][i] == n_win  ||
	  ...
	  // The 2 possibilities are disjoint:
	  assert(!(table[player][i] == n_win)  ||
		 !(table[player][i] == n_loss  &&  move_count[player][i] == 0));
	  ...
	  for (uint j=0; j<retro_moves.size(); j++) {
	    ...
	    bool reduced_symmetry_before = !pawnless_endgame ? false :
	      (SAME_DIAGONAL[board.get_king_position(0)] ==
	       SAME_DIAGONAL[board.get_king_position(1)]);
	    
	    pair<uint, int> back = get_table_index_and_stm(board);
	    ...
	    if (!reduced_symmetry  &&  reduced_symmetry_before) {
	      ...
	      pair<uint, int> back2 = get_table_index_and_stm(board);
