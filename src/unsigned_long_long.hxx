...
// (the ull struct is mainly created because unsigned long long gave
//  warnings when compiled with pedantic etc.)
#ifdef USE_LONG_LONG

typedef unsigned long long ull;
ull& operator|=(ull& lval, int bit_pos) {
  lval |= 1<<bit_pos;
  return lval;
}
bool operator[](const ull& lval, int bit_pos) const {
  return (lval>>bit_pos) & 1;
}
