...
public:
  ...
  uint operator[](int index) {
    ...
    int low = index & 31;
    index >>= 5;
    uint result = mem[index] >> low;
    if (low+bits_per_value & 32) {
      return ((mem[index+1]<<(32-low)) | result) & ONE_BITS[bits_per_value];
    } else {
      return result & ONE_BITS[bits_per_value];
    }
    */
  }
