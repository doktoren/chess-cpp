...
extern const int uint8_log[256];
extern const int bit_count[256];
extern const uint8_t uint8_hash[256];

inline int floor_log(uint n) {
  if (n & 0xFFFF0000) {
    if (n & 0xFF000000) return 24 + uint8_log[n >> 24];
    return 16 + uint8_log[n >> 16];
  } else {
    if (n & 0xFF00) return 8 + uint8_log[n >> 8];
    return uint8_log[n];
  }
}
