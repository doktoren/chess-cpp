...
void obstream<OUTPUT_MODEL>::writeEliasNumber(uint n) {
  ...
  if (n>3) {
    ...
    OUTPUT_MODEL::writeBit(f_log & 1);
    OUTPUT_MODEL::writeBit(!(f_log & 1));

    OUTPUT_MODEL::writeNBit(f_log, n);
  } else {
    OUTPUT_MODEL::writeBit(n!=1);
    if (n!=1) OUTPUT_MODEL::writeBit(n&1);
  }
}
...
void obstream<OUTPUT_MODEL>::writeFixedIntervalNumber(uint succ_max, uint n) {
  ...
  if (n < tmp) OUTPUT_MODEL::writeNBit(c_log-1, n);
  else OUTPUT_MODEL::writeNBit(c_log, n+tmp);
}
...
void obstream<OUTPUT_MODEL>::writeNumber(uint expected_size, uint number) {
  ...
  while (number >> num_bits) {//1<<num_bits <= number) {
    OUTPUT_MODEL::writeBit(1);
    ++num_bits;
  }
  OUTPUT_MODEL::writeBit(0);
  
  OUTPUT_MODEL::writeNBit(num_bits, number);
}
...
void obstream<OUTPUT_MODEL>::writeArbitraryNumber(uint n) {
  ...
  while (unary_length_specifier[++i] < c_log)
    OUTPUT_MODEL::writeBit(1);
  OUTPUT_MODEL::writeBit(0);
  OUTPUT_MODEL::writeNBit(unary_length_specifier[i], n);
}
