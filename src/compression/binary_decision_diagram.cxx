...
void BinaryDecisionDiagram::init(const TYPE *table, int _log_size, uint max_value,
  ...
  ushort *_table = (ushort *)malloc(sizeof(ushort)*(size>>1));
  ...
  // layer == 1
#define __table ((uint *)_table)
  {
    int layer = 1;
    int _size = size>>layer;

    map<pair<ushort, ushort>, uint> mapping;

    for (int i=0; i<_size; i+=2) {
      pair<ushort, ushort> tmp(_table[i], _table[i+1]);
      if (mapping.count(tmp)) {
	// Use 2 ushort entries in _table as one uint entry!
  ...
  for (int layer=2; layer<log_size; layer++) {
    ...
    realloc(_table, sizeof(uint)*(_size>>1));
    assert(_table);
...
#undef __table
  ...
  if (calc_sifting) {
	...
	cbo << "(" << i << "," << (int)bit_perm[i] << "),";
      ...
      // Kør alle variabler igennem en gang
      for (int bit_num = 0; bit_num<log_size; bit_num++) {
	// Find bittens placering
	int pos = bit_perm[bit_num];// corrected from inv_bit_perm

	int best_pos = pos;
	int cur_value = 0; // compared to best_pos
	
	// Todo: Average speed improvement can be archieved by first shifting
	// the bit towards the nearest "endpoint" (ie 0 iff pos < log_size/2)
